...
export const signUpWithEmail = async (email, password, name, metadata = {}) => {
  try {
    ...
    // Store user type in cookie so auth callback can read it when user confirms email
    // This is needed because the callback is server-side and needs to know the user type
    const userType = metadata.type || 'tenant';
    if (typeof window !== 'undefined') {
      localStorage.setItem('pending_user_type', userType);
      // Set cookie that expires in 24 hours (user might not confirm email immediately)
      document.cookie = `pending_user_type=${userType}; path=/; max-age=86400; SameSite=Lax`;
    }
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      options: {
        data: {
          name,
          type: userType, // Ensure type is in metadata
          ...metadata
        },
...
export const resetPassword = async (email) => {
  try {
    ...
    // Use direct Supabase client call (most reliable method)
