/**
 * DEPRECATED: This file has been replaced with Supabase
 * Use lib/database.js instead for all database operations
 * 
...
const deprecationError = () => ({
  success: false,
  error: 'This function has been deprecated. Please use lib/database.js with Supabase instead.'
});
...
export const checkPhoneNumberExists = async (phoneNumber) => {
  console.warn('⚠️ checkPhoneNumberExists is deprecated. Use database.js instead.');
...
// ============================================
// WALLET OPERATIONS (Using transactions for atomicity)
// ============================================
...
export const addCredits = async (userId, amount, reason = 'Credit Purchase') => {
  try {
    ...
    // Record transaction (outside of Firestore transaction to avoid complexity)
...
export const trackAgentLeadContact = async (agentId, leadId, contactType) => {
  try {
    if (!agentId || !leadId) {
      return { success: false, error: 'Missing agentId or leadId' };
    }

    const contactRef = doc(db, 'requests', leadId, 'agentContacts', agentId);
    const leadRef = doc(db, 'requests', leadId);
    ...
    return await runTransaction(db, async (transaction) => {
      ...
      if (contactDoc.exists()) {
        transaction.update(contactRef, {
          ...
          contactTypes: arrayUnion(contactType)
        });
        return { success: true, isNewContact: false };
      }
      ...
      return { success: true, isNewContact: true };
    });
  } catch (error) {
    console.error('Error tracking agent contact:', error);
    return { success: false, error: error.message };
  }
};
...
export const hasAgentContactedLead = async (agentId, leadId) => {
  try {
    if (!agentId || !leadId) {
      return { success: false, hasContacted: false };
    }
    
    const contactRef = doc(db, 'requests', leadId, 'agentContacts', agentId);
    const contactDoc = await getDoc(contactRef);
    
    return { success: true, hasContacted: contactDoc.exists() };
  } catch (error) {
...
export const searchProperties = async (searchParams) => {
  try {
    ...
    // Note: Firestore requires orderBy on the field used in inequality filters first
    // If using range queries on price, we must order by price, not createdAt
    const hasPriceFilter = searchParams.minPrice || searchParams.maxPrice;
    ...
    // Order by price if we have price filters, otherwise by createdAt
    if (hasPriceFilter) {
    ...
    // If we sorted by price, also sort by createdAt in memory for consistent ordering
    if (hasPriceFilter) {
      properties.sort((a, b) => {
        // First by price, then by createdAt descending
...
export const checkSubscriptionStatus = async (agentId) => {
  try {
    ...
    if (result.success) {
      ...
      // Safely handle endDate - could be Firestore Timestamp, Date, or null
      let endDate;
      if (subscription.endDate) {
        ...
        // If no endDate, treat as expired
  ...
  try {
    ...
    if (!snapshot.empty) {
      ...
      if (subscription.endDate) {
        ...
        // If no endDate, treat as expired
        return { success: false, error: 'Invalid subscription data', isActive: false };
      }
...
const createTransaction = async (userId, type, amount, description, metadata = {}) => {
  try {
    await addDoc(collection(db, 'transactions'), {
      userId,
      type, // 'credit_purchase', 'credit_add', 'credit_deduct', 'lead_unlock', 'bonus', 'refund'
      amount,
      description,
      metadata,
      createdAt: serverTimestamp()
