...
export const createLead = async (leadData) => {
  try {
    ...
    // --- NOTIFICATION LOGIC ---
    try {
      // Find all agents to check for location overlap
...
export const incrementLeadViews = async (leadId, agentId) => {
  ...
  try {
    ...
    // Log the initial view in contact_history with 0 cost
    // This serves as our "unique view" record
...
export const unlockLead = async (agentId, leadId) => {
  try {
    ...
    // --- NOTIFICATION LOGIC ---
    try {
      // 1. Get Lead to find the Tenant ID
      const { data: lead } = await supabase.from('leads').select('user_id').eq('id', leadId).single();

      // 2. Get Agent to find their Name
      const { data: agent } = await supabase.from('users').select('name').eq('id', agentId).single();
      ...
      if (lead?.user_id && agent?.name) {
        await createNotification({
          ...
          message: `${agent.name} has viewed your rental request and may contact you soon.`,
          data: { agentName: agent.name, leadId }
        });
      }
    } catch (notifErr) {
      console.error('Non-critical error sending unlock notification:', notifErr);
    }
    // ---------------------------
...
export const getUnlockedLeads = async (agentId) => {
  try {
    ...
    console.log(`getUnlockedLeads for agent ${agentId}:`, leadIds.length, 'leads', leadIds);
...
export const getAgentConnectedLeads = async (agentId, filters = {}) => {
  try {
    ...
    // Query contact_history to get all leads the agent has unlocked
    let query = supabase
      .from('contact_history')
      .select(`
        id,
