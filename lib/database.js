...
export const unlockLead = async (agentId, leadId, isExclusive = false) => {
  try {
    ...
    // 7. Store the connection (VITAL for persistence and UI state)
...
export const checkAgentLeadConnection = async (leadId, agentId) => {
  try {
    ...
    // 1. Check primary connection table
    const { data, error } = await supabase
    ...
    if (error) throw error;
    if (data) return { success: true, connected: true };

    // 2. Fallback to contact_history for legacy data or redundancy
