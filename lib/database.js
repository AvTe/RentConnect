 ...
 * Prevents SQL injection through search terms
 * @param {string} str - Input string to escape
 * @returns {string} - Escaped string safe for ilike patterns
 */
export const escapeIlikePattern = (str) => {
  if (!str) return '';
  // Escape special PostgreSQL LIKE/ILIKE pattern characters: %, _, \
...
export const getWalletTransactions = async (userId, options = {}) => {
  try {
    ...
    const transactions = (data || []).map(tx => ({
      ...
      type: tx.type === 'credit' ? 'topup' : tx.reason?.toLowerCase().includes('referral') ? 'referral' : 'unlock',
      description: tx.reason || (tx.type === 'credit' ? 'Credit Purchase' : 'Lead Unlock'),
  ...
  try {
    ...
    recentUsers?.forEach(user => {
      activities.push({
        ...
        description: `${user.name} joined Yoombaa`,
...
export const getAdminUsers = async (filters = {}, pagination = {}) => {
  try {
    ...
    // Apply filters (escape search term to prevent SQL injection)
    if (search) {
...
/**
 * Simple hash function for non-password tokens (invite tokens, etc.)
 * @param {string} token
 * @returns {string}
 */
const hashToken = (token) => {
  // Use crypto for non-password token hashing
...
export const acceptAdminInvite = async (token, password) => {
  try {
    ...
    // Update admin user status and password using bcrypt
...
/**
 * Get all bad lead reports (admin only)
 * @param {object} filters - Filter options
 * @param {string} requesterId - ID of user making the request (for admin verification)
 * @returns {Promise<{success: boolean, data?: array, error?: string}>}
 */
export const getBadLeadReports = async (filters = {}, requesterId = null) => {
  try {
    const supabase = createClient();
    const { status, limit = 50, offset = 0 } = filters;

    // Verify requester is an admin if requesterId is provided
    if (requesterId) {
      ...
      const isAdmin = ['admin', 'super_admin', 'main_admin', 'sub_admin'].includes(requester?.role);
 ...
 * @returns {Promise<{success: boolean, data?: array, error?: string}>}
 */
export const getRevenueTrendData = async (days = 30) => {
  try {
    const supabase = createClient();
    ...
    data?.forEach(tx => {
      ...
      grouped[date] = (grouped[date] || 0) + parseFloat(tx.amount || 0);
    });
    ...
    if (days > 30) {
      ...
      for (let i = 0; i < result.length; i += 7) {
        const week = result.slice(i, i + 7);
        const total = week.reduce((sum, d) => sum + d.revenue, 0);
        weeklyData.push({ date: week[0].date, revenue: total });
      }
...
export const getUserGrowthData = async (days = 30) => {
  try {
    ...
    users?.forEach(user => {
      ...
      if (user.role === 'agent') {
        agentsByDate[date] = (agentsByDate[date] || 0) + 1;
      } else if (user.role === 'tenant') {
        tenantsByDate[date] = (tenantsByDate[date] || 0) + 1;
      }
    });
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      d.setDate(d.getDate() - i);
      const dateStr = d.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
      cumulativeAgents += agentsByDate[dateStr] || 0;
      cumulativeTenants += tenantsByDate[dateStr] || 0;
      result.push({ date: dateStr, agents: cumulativeAgents, tenants: cumulativeTenants });
    }
 ...
 * Properly checks expires_at to count expired leads accurately
 * @returns {Promise<{success: boolean, data?: array, error?: string}>}
 */
export const getLeadsDistribution = async () => {
  try {
    const supabase = createClient();
    const now = new Date().toISOString();

    // Fetch all leads with status and expires_at
    const { data: leads, error } = await supabase
      .from('leads')
    ...
    // Count leads by effective status (checking expires_at for true expiration)
    let active = 0;
    let paused = 0;
    let expired = 0;
    let closed = 0;
    ...
    leads?.forEach(lead => {
      const isExpired = lead.expires_at && new Date(lead.expires_at) < new Date(now);

      if (lead.status === 'closed') {
        closed++;
      } else if (lead.status === 'expired' || isExpired) {
        expired++;
      } else if (lead.status === 'paused') {
        paused++;
      } else {
        active++;
      }
    ...
    console.error('Error getting leads distribution:', error);
    return { success: false, error: error.message };
  }
};

/**
 * Get top locations for leads
 * @param {number} limit - Number of locations to return
 * @returns {Promise<{success: boolean, data?: array, error?: string}>}
 */
export const getTopLeadLocations = async (limit = 10) => {
  try {
    const supabase = createClient();
    ...
    data?.forEach(lead => {
      const loc = lead.location?.trim() || 'Unknown';
    ...
    console.error('Error getting top lead locations:', error);
    return { success: false, error: error.message };
  }
};

/**
 * Get daily activity data (signups, transactions, leads)
 * @param {number} days - Number of days to fetch
 * @returns {Promise<{success: boolean, data?: array, error?: string}>}
 */
export const getDailyActivityData = async (days = 7) => {
  try {
    const supabase = createClient();
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - days);
    ...
    const [usersResult, txResult, leadsResult] = await Promise.all([
      supabase.from('users').select('created_at').gte('created_at', startDate.toISOString()),
      supabase.from('credit_transactions').select('created_at').gte('created_at', startDate.toISOString()),
      supabase.from('leads').select('created_at').gte('created_at', startDate.toISOString())
    ]);
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      result.push({
        ...
        transactions: txByDay[dayStr] || 0,
        leads: leadsByDay[dayStr] || 0
      });
    }
...
/**
 * Get revenue breakdown by bundle type
 * @param {number} days - Number of days to fetch
 * @returns {Promise<{success: boolean, data?: array, error?: string}>}
 */
export const getRevenueBreakdown = async (days = 30) => {
  try {
    const supabase = createClient();
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - days);
    ...
    // Group by week and bundle type
    const weeklyData = {};

    data?.forEach(tx => {
      const weekStart = new Date(tx.created_at);
      weekStart.setDate(weekStart.getDate() - weekStart.getDay());
      const weekKey = weekStart.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });

      if (!weeklyData[weekKey]) {
        weeklyData[weekKey] = { date: weekKey, starter: 0, basic: 0, pro: 0, enterprise: 0 };
      }

      const amount = parseFloat(tx.amount || 0);
      const reason = (tx.reason || '').toLowerCase();

      if (reason.includes('starter') || amount <= 100) {
        weeklyData[weekKey].starter += amount;
      } else if (reason.includes('basic') || amount <= 300) {
        weeklyData[weekKey].basic += amount;
      } else if (reason.includes('pro') || amount <= 600) {
        weeklyData[weekKey].pro += amount;
      } else {
        weeklyData[weekKey].enterprise += amount;
      }
    });
 ...
 * @returns {Promise<{success: boolean, data?: array, error?: string}>}
 */
export const getTransactionVolume = async (days = 30) => {
  try {
    const supabase = createClient();
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - days);
    ...
    data?.forEach(tx => {
      ...
      grouped[date].volume += parseFloat(tx.amount || 0);
    });
