  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-black text-gray-900">Tenant Management</h1>
          <p className="text-sm text-gray-500 font-medium mt-1">Monitor and manage tenant accounts and leads</p>
        </div>
          ...
          <div className="flex bg-gray-100 rounded-xl p-1">
            {[
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${statusFilter === f.id ? 'bg-[#FE9200] text-white' : 'text-gray-600 hover:text-gray-900'
                  }`}
              >
                {f.label}
              </button>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        ...
        <div className="lg:hidden divide-y divide-gray-100">
          {loading ? (
            ...
            paginatedRenters.map((renter) => (
              <div key={renter.id} className="p-4">
                ...
                <div className="grid grid-cols-2 gap-3 text-xs bg-gray-50 rounded-xl p-3">
                  ...
                  <div>
                    ...
                    <span className="font-bold text-gray-900">{renter.location || 'N/A'}</span>
                  </div>
                </div>
        ...
        {filteredRenters.length > perPage && (
          <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
            <span className="text-sm text-gray-500">
              Showing {(page - 1) * perPage + 1} to {Math.min(page * perPage, filteredRenters.length)} of {filteredRenters.length}
            </span>
